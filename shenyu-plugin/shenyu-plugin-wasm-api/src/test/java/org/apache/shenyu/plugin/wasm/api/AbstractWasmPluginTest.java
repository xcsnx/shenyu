...
//    private RustWasmPlugin rustWasmPlugin;
    ...
    private GoWasmPlugin goWasmPlugin;
...
//        this.rustWasmPlugin = spy(new RustWasmPlugin());
        this.goWasmPlugin = spy(new GoWasmPlugin());
...
//        verify(shenyuPluginChain).execute(exchange);
        ...
        StepVerifier.create(goWasmPlugin.execute(exchange, shenyuPluginChain)).expectSubscription().verifyComplete();
        verify(shenyuPluginChain).execute(exchange);
    }
...
//    }
    ...
    static class GoWasmPlugin extends AbstractWasmPlugin {
        ...
        protected Map<String, Func> initWasmCallJavaFunc(final Store<Void> store) {
            Map<String, Func> funcMap = new HashMap<>();
            funcMap.put("add", WasmFunctions.wrap(store, WasmValType.I64,WasmValType.I64, WasmValType.I32,
                (addr, len) -> {
                    ByteBuffer buf = super.getBuffer();
